<template>
  <div>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                ...
                <li v-for="(item,index) in addressList" :key="index" :class="{'check':checkIndex == index}" @click="checkIndex = index;selectedAddrId=item._id">
                  <dl>
                    <dt>{{item.userName}}</dt>
                  ...
                  <div class="addr-opration addr-del">
                    <a href="javascript:;" class="addr-del-btn" @click="delress(item._id)">
                      <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                    </a>
                  </div>
                  <div class="addr-opration addr-set-default">
                    <a href="javascript:;" class="addr-set-default-btn" v-if="!item.isDefault" @click="setDefault(item._id)"><i>设置默认地址</i></a>
                  </div>
                ...
                <li class="addr-new" @click="address()">
          ...
          <div class="lemall-msg-info hidden">
            <span>您选择的区域不在我们的送货区。请选择我们送货区内的一个送货地址。</span>
          </div>
    ...
    <modal :mdShow="addressshow">
      <span href="" slot="message">
        请输入地址信息
      </span>
      <div slot="in">
        <div class="form-group">
        <label class="control-label">姓名:</label>
        <input type="text" class="form-control input-sm" v-model="newress.userName">
      </div>
      <div class="form-group">
        <label  class="control-label">地址:</label>
        <input type="text" class="form-control input-sm" v-model="newress.streetName">
      </div>
      <div class="form-group">
        <label class="control-label">邮编:</label>
        <input type="text" class="form-control input-sm" v-model="newress.postCode">
      </div>
        <div class="form-group">
        <label class="control-label">电话:</label>
        <input type="text" class="form-control input-sm" v-model="newress.tel">
        </div>
      </div>
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="newaddress">确定</a>
        <a href="javascript:;" class="btn btn--m" @click="addressshow = false">关闭</a>
      </div>
    </modal>
    <modal :mdShow="addtrue">
      <span href="" slot="message">
        添加成功
      </span>
    <div slot="btnGroup">
      <a href="javascript:;" class="btn btn--m" @click="addtrue = false">关闭</a>
    </div>
  </modal>
    <modal :mdShow="addfalse">
      <span href="" slot="message">
        添加失败
      </span>
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="addfalse = false">关闭</a>
      </div>
    </modal>
    <modal :mdShow="resstransfer">
      <span href="" slot="message">
        不能有为空项
      </span>
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="resstransfer = false">关闭</a>
      </div>
    </modal>
    <modal :mdShow="realydel">
      <span href="" slot="message">
        确定删除？
      </span>
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="realydelress">确定</a>
        <a href="javascript:;" class="btn btn--m" @click="realydel = false">关闭</a>
      </div>
    </modal>
  </div>
</template>
...
<script>
  ...
  import PubSub from 'pubsub-js'
    export default {
      ...
      mounted() {
        this.init();
        PubSub.subscribe('refresh', (msg,index) => {
          init()
        })
      },
      methods:{
        ...
        init(){
          ...
          this.$http.get('/users/addressList')
            ...
            this.addressList.forEach((item,index) => {
              ...
              this.selectedAddrId = this.addressList[checkI]._id
            })
        ...
        newaddress(){
          let i = 0;
          Object.keys(this.newress).forEach(key => {
            if(this.newress[key] == ''){
                this.resstransfer = true
                return
            }else{
              i++;
            }
            if(i == 4) {
              this.$http.post('/users/addnewress', {newress: this.newress})
                .then(res => {
                  if (res.status == 1) {
                    this.addfalse = true
                  } else {
        ...
        realydelress(){
          ...
          this.$http.post('/users/delress',{_id:this._id}).then(
            res => {
              this.realydel = false
              this.init()
            })
